...
class TokenRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setManaged($managed)
    {
        ...
        if (!is_null($managed) && (mb_strlen($managed) < 1)) {
            throw new \InvalidArgumentException('invalid length for $managed when calling TokenRequest., must be bigger than or equal to 1.');
        }
    ...
    public function setIdentifier($identifier)
    {
        ...
        if ((mb_strlen($identifier) > 255)) {
            throw new \InvalidArgumentException('invalid length for $identifier when calling TokenRequest., must be smaller than or equal to 255.');
        }
        if ((mb_strlen($identifier) < 1)) {
            throw new \InvalidArgumentException('invalid length for $identifier when calling TokenRequest., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/^[-a-zA-Z0-9_]+$/", ObjectSerializer::toString($identifier)))) {
            throw new \InvalidArgumentException("invalid value for \$identifier when calling TokenRequest., must conform to the pattern /^[-a-zA-Z0-9_]+$/.");
        }
